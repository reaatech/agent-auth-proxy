...
describe.skipIf(!(await isDbAvailable()))('OAuth2Manager', () => {
  ...
  it('should throw for unknown provider', async () => {
    await expect(manager.initiateAuthorization('user-123', 'unknown', ['read'])).rejects.toThrow(
      "OAuth provider 'unknown' not configured",
    );
  });
  ...
  it('should initiate authorization and store state', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'oauth-test@example.com' })
      .onConflictDoNothing();
  ...
  it('should store and retrieve tokens', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'token-test@example.com' })
      .onConflictDoNothing();
  ...
  it('should check required scopes', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'scope-test@example.com' })
      .onConflictDoNothing();
  ...
  it('should refresh expired token', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'refresh-test@example.com' })
      .onConflictDoNothing();
    await manager.storeTokens(userId, 'mock', {
  ...
  it('should revoke token', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'revoke-test@example.com' })
      .onConflictDoNothing();
    await manager.storeTokens(userId, 'mock', {
  ...
  it('should handle callback and exchange code', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'callback-test@example.com' })
      .onConflictDoNothing();
  ...
  it('should exchange form-encoded token response', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'form-test@example.com' })
      .onConflictDoNothing();
    const { state } = await manager.initiateAuthorization(userId, 'mock', ['read']);
    ...
    vi.stubGlobal('fetch', async (url: string | URL) => {
      ...
      if (urlStr.includes('userinfo')) {
        return new Response(JSON.stringify({ id: 'u1', email: 'form-test@example.com' }), {
          status: 200,
  ...
  it('should fail refresh when token endpoint returns error', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'refresh-fail@example.com' })
      .onConflictDoNothing();
    await manager.storeTokens(userId, 'mock', {
      access_token: 'access-old',
      token_type: 'Bearer',
  ...
  it('should fail token exchange when endpoint returns error', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'exchange-fail@example.com' })
      .onConflictDoNothing();
    const { state } = await manager.initiateAuthorization(userId, 'mock', ['read']);
    ...
    await expect(manager.handleCallback('bad-code', state, 'mock')).rejects.toThrow(
  ...
  it('should retrieve token with missing metadata authTag fallback', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'metadata-test@example.com' })
      .onConflictDoNothing();
    const token = await manager.storeTokens(userId, 'mock', {
    ...
    await db
      ...
      .set({
        metadata: {
          accessTokenAuthTag: (token.metadata as Record<string, string>).accessTokenAuthTag,
        },
      })
  ...
  it('should preserve refresh token during refresh when not rotated', async () => {
    await db
      .insert(users)
      .values({ id: userId, email: 'preserve-refresh@example.com' })
      .onConflictDoNothing();
    await manager.storeTokens(userId, 'mock', {
