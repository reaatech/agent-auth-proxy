...
describe('authenticateAgent', () => {
  ...
  it('should return 401 when JWT payload missing agentId', async () => {
    ...
    expect(reply.code).toHaveBeenCalledWith(401);
    expect(reply.send).toHaveBeenCalledWith(expect.objectContaining({ error: 'AUTH_REQUIRED' }));
  });
});
...
describe('requireAdmin', () => {
  it('should return 403 when admin key is missing', async () => {
    ...
    expect(reply.send).toHaveBeenCalledWith(expect.objectContaining({ error: 'ADMIN_REQUIRED' }));
  });
});
...
describe('authenticateAgentWithApiKey', () => {
  it('should return 401 when authorization header missing', async () => {
    ...
    expect(reply.code).toHaveBeenCalledWith(401);
    expect(reply.send).toHaveBeenCalledWith(expect.objectContaining({ error: 'INVALID_API_KEY' }));
  });
