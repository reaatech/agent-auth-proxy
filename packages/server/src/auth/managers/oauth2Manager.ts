...
export class OAuth2Manager {
  ...
  private async exchangeCodeForTokens(
    ...
    if (
      ...
      return {
        ...
        expires_in: Number.parseInt(parsed.get('expires_in') || '3600', 10),
  ...
  async storeTokens(
    ...
    if (tokenResponse.refresh_token) {
      const refreshResult = await this.encryptionService.encrypt(
  ...
  async getValidToken(
    ...
    const accessTokenAuthTag =
      token.accessTokenAuthTag ||
  ...
  private async doRefreshAccessToken(userId: string, provider: string): Promise<string> {
    ...
    const refreshTokenAuthTag =
      token.refreshTokenAuthTag ||
      (token.metadata as Record<string, string> | null)?.refreshTokenAuthTag ||
      '';

    if (!token.refreshTokenIv) {
      throw new Error('Refresh token IV missing — token cannot be decrypted');
    }
    ...
    const newAccessTokenAuthTag =
      newToken.accessTokenAuthTag ||
