...
import { z } from 'zod';
import { AppError, AuthError, ScopeError, UpstreamError, ValidationError } from '@/utils/errors';
import { authenticateAgent } from '@/api/middleware/auth';
import { proxyParamsSchema } from '@/api/middleware/validation';
      ...
      try {
        ...
        if (!scopeResult.allowed) {
          ...
          throw new ScopeError(
            ...
            { grantedScopes: scopeResult.grantedScopes, requestedScopes: scopeResult.requestedScopes },
          );
        }
        ...
        for (const [key, value] of Object.entries(request.headers)) {
          ...
          if (
            ['host', 'authorization', 'x-user-id', 'x-admin-api-key', 'content-length',
              'cookie', 'x-forwarded-for', 'x-forwarded-proto', 'x-forwarded-host',
              'x-forwarded-port', 'x-real-ip'].includes(lowerKey)
          )
            continue;
        ...
        if (downstreamResponse.status === 401 && credential.type === 'bearer') {
          if (!['GET', 'HEAD', 'OPTIONS'].includes(request.method)) {
            ...
            return forwardResponse(
              downstreamResponse, reply, startTime, userId, agentId, provider, path, auditLogger,
            );
          }
          ...
          return forwardResponse(
            retryResponse, reply, startTime, userId, agentId, provider, path, auditLogger,
          );
        }
        ...
        if (error instanceof AuthError || error instanceof ScopeError || error instanceof ValidationError) {
          throw error;
        }
...
async function forwardResponse(
  ...
  const strippedHeaders = ['set-cookie', 'www-authenticate', 'x-amz-id-2', 'x-amz-request-id',
    'x-frame-options', 'content-security-policy', 'x-powered-by'];
  ...
  const logAudit = (outcome: 'success' | 'failure', statusCode: number, details?: Record<string, unknown>) =>
    auditLogger.log({
      eventType: 'api_call',
      userId,
      agentId,
  ...
  if (isStream || (response.body && contentLength !== null && contentLength > config.proxyStreamThresholdBytes)) {
