...
describe.skipIf(!(await isDbAvailable()))('Proxy Endpoints', () => {
  ...
  async function getAgentToken(): Promise<{ userId: string; agentId: string; token: string }> {
    const userRes = await app.post('/api/v1/users').set('X-Admin-API-Key', adminKey).send({ email: `agent-${Date.now()}@example.com` });
    const agentRes = await app.post('/api/v1/agents').set('X-Admin-API-Key', adminKey).send({ name: `agent-${Date.now()}` });
    const authRes = await app.post('/auth/agent').set('Authorization', `Bearer ${agentRes.body.api_key}`);
  ...
  it('should return 400 without X-User-ID', async () => {
    ...
    const res = await app.get('/proxy/google/calendar/v3/calendars').set('Authorization', `Bearer ${token}`);
  ...
  it('should forward POST body to downstream', async () => {
    ...
    expect(JSON.parse(capturedBody!)).toEqual({ summary: 'Test Event', start: { dateTime: '2024-01-01T00:00:00Z' } });
  });
