...
describe.skipIf(!(await isDbAvailable()))('Auth Endpoints', () => {
  ...
  it('should return 400 for invalid authorize query', async () => {
    const res = await app.get('/oauth/authorize').query({ user_id: 'bad-uuid', provider: 'google', scopes: 'email' });
  ...
  it('should return 400 for unknown provider in authorize', async () => {
    const res = await app
      .get('/oauth/authorize')
      .query({ user_id: '00000000-0000-0000-0000-000000000001', provider: 'unknown', scopes: 'email' });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('OAUTH_INITIATE_FAILED');
  });

  it('should initiate authorization for valid request', async () => {
    await db.insert(users).values({ id: '00000000-0000-0000-0000-000000000003', email: 'auth@example.com' }).onConflictDoNothing();
    const res = await app
      .get('/oauth/authorize')
      .query({ user_id: '00000000-0000-0000-0000-000000000003', provider: 'google', scopes: 'email,profile' });
  ...
  it('should handle callback with valid state', async () => {
    ...
    await db.insert(users).values({ id: userId, email: 'callback@example.com' }).onConflictDoNothing();
    ...
    vi.stubGlobal('fetch', async (url: string | URL) => {
      ...
      if (urlStr.includes('userinfo')) {
        return new Response(
          JSON.stringify({ id: 'u1', email: 'callback@example.com' }),
          { status: 200, headers: { 'content-type': 'application/json' } },
        );
      }
