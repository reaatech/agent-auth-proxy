...
import { users, agents, userAgentGrants, scopes as scopesTable } from '@/db/schema';
...
describe.skipIf(!(await isDbAvailable()))('ScopeEnforcer', () => {
  ...
  describe('validation', () => {
    ...
    async function createScope(name: string, provider: string, riskLevel: string = 'low') {
      await db.insert(scopesTable).values({ name, provider, category: 'read', riskLevel }).onConflictDoNothing();
    }
  ...
  describe('escalation and caching', () => {
    ...
    it('should detect scope escalation for exceeding risk level', async () => {
      await db.insert(users).values({ id: userId, email: 'escalation@example.com' }).onConflictDoNothing();
      await db.insert(agents).values({
        id: agentId, name: 'escalation-agent', active: true,
        apiKeyHash: 'hash', apiKeyPrefix: 'pref',
    ...
    it('should use cache on second request', async () => {
      await db.insert(users).values({ id: userId, email: 'cache@example.com' }).onConflictDoNothing();
      await db.insert(agents).values({ id: agentId, name: 'cache-agent', active: true, apiKeyHash: 'hash', apiKeyPrefix: 'pref' }).onConflictDoNothing();
      await db.insert(scopesTable).values({ name: 'read', provider: 'test', category: 'data', riskLevel: 'low' }).onConflictDoNothing();
