...
describe.skipIf(!(await isDbAvailable()))('KeyVault', () => {
  ...
  beforeEach(async () => {
    ...
    await db.insert(users).values({ id: userId, email: 'vault-test@example.com' }).onConflictDoNothing();
  });
  ...
  it('should throw for missing key', async () => {
    await expect(vault.getApiKey('00000000-0000-0000-0000-000000009999', 'nonexistent')).rejects.toThrow(
      'No valid API key',
    );
  });
